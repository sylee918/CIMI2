...
if ispec eq 0 then fhead='CimiTest_h'
if ispec eq 1 then fhead='CimiTest_o'
if ispec eq 2 then fhead='CimiTest_e'

header=''
openr,2,fhead+'.fls'
readf,2,header   	 ; header
    ...
    for i=0,nr-1 do begin
        for j=0,nmlt-1 do begin
            ...
            for k=0,je-1 do begin
                if (k eq 0) then readf,2,header   	 ; header
                readf,2,fy
...
if            iiter eq 0 then begin
   ie1= 0 & ie2= 5
   fmax=5.  & fmin=2.
endif else if iiter eq 1 then begin
   ie1= 6 & ie2=10
   fmax=5.3 & fmin=2.3
endif else if iiter eq 2 then begin
   ie1=11 & ie2=14
   fmax=5.3 & fmin=2.3
endif else if iiter eq 3 then begin
   ie1=15 & ie2=17
   fmax=4.6 & fmin=1.6
endif else if iiter eq 4 then begin
   ie1=18 & ie2=20
   fmax=4.6 & fmin=1.6
endif else begin
   ie1=21 & ie2=23
...
for n=0,nmax do begin
  ...
  ;     ytitle=y_title,xrange=[hour_min,hour_max],xstyle=1, $
    contour,plsfluxs,minute,Lshell0,yrange=[Lmin,Lmax],ystyle=1,xtitle='time (min)', $
        ytitle=y_title,xrange=[0.,2.],xstyle=1, $
